...
namespace App\Http\Controllers\Distribution\Supplier;
...
class ProduitsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('distribution.supplier.produits.index', compact('lists', 'suppliers', 'scenics'));
    }
    ...
    public function create(Request $request)
    {
        try {
            ...
            if ($request->isMethod('POST')) {
                ...
                if ($validator->fails()) {
                    echo "效验失败";
                    return redirect('/distribution/supplier/produits/create')
                        ->withInput()
                ...
                $produits->save();
                if ($produits) {
                    return redirect('/distribution/supplier/produits')->withSuccess('保存成功！');
                }
            ...
            return view('distribution.supplier.produits.create', compact('produits', 'products', 'suppliers', 'scenics'));
    ...
    public function original($id, Request $request)
    {
        try {
            ...
            if ($request->isMethod('POST')) {
                ...
                if ($validator->fails()) {
                    echo "效验失败";
                    return redirect('/distribution/supplier/produits/original/' . $id)
                        ->withInput()
                        ->withErrors($validator);
                }
            ...
            $scenics = Scenic::all();
            return view('distribution.supplier.produits.original', compact('produits', 'scenics'));
